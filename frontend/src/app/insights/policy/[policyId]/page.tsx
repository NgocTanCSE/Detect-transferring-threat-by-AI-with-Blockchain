      ...
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-400">Policy ID: {policyId}</p>
          </div>
          <div className="text-xs text-zinc-500">
            <Link href={backHref} className="hover:text-zinc-300">Dashboard</Link>
            <span className="mx-2">/</span>
            <span className="text-zinc-300">Policy insight</span>
          </div>
        ...
        {isLoading ? <p className="text-zinc-400">Loading policy...</p> : null}
        {error ? <p className="text-red-300">Cannot load policy data.</p> : null}
        {!isLoading && !error && !policy ? <p className="text-zinc-400">Policy not found.</p> : null}
        ...
        {policy ? (
          <div className="space-y-4">
            <section className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
              ...
              <p className="mt-2 text-sm text-zinc-300">{policy.description || "No description"}</p>
            </section>
            ...
            <section className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
              ...
              <p className="mt-4 text-xs text-zinc-500">Created: {formatDateTime(policy.created_at)}</p>
              <p className="text-xs text-zinc-500">Updated: {formatDateTime(policy.updated_at)}</p>
            </section>
          </div>
        ) : null}
      </div>
